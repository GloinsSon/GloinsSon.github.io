...
export class ViewController {
     ...
     */
    buildHeart(type, index, color, disabled) {
        let template =
            "<label for='{type}{index}'>" +
            "    <input type='radio' id='{type}{index}' name='{type}' value='{index}' {disabled} />" +
     ...
     * @returns {string}
     */
    buildLogo(type, value, folder, image, disabled) {
        const template =
            "<label>" +
     ...
     * @returns {string}
     */
    buildExtra(name, species, value, filename, disabled) {
        const template =
            "<label>" +
